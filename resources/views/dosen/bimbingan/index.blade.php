...
@section('content')
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        ...
        <nav class="text-sm font-medium text-gray-500 bg-gray-100 px-4 py-2 rounded-lg">
            <ol class="list-reset flex items-center gap-2">
                <li><a href="{{ route('dosen.dashboard') }}" class="hover:text-green-600 transition">Dashboard</a></li>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        @forelse ($bimbingans as $bimbingan)
            <div class="bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow duration-300 border border-gray-100 overflow-hidden flex flex-col h-full"
                ...
                <div class="p-6 flex-1 flex flex-col">
                    <div class="flex justify-between items-start mb-4">
                        <div class="flex items-center gap-3">
                            <div class="h-10 w-10 rounded-full bg-gray-100 flex items-center justify-center text-gray-600 font-bold text-lg">
                                {{ substr($bimbingan->mahasiswa->name, 0, 1) }}
                            </div>
                            <div>
                                <h3 class="font-bold text-gray-800 text-lg leading-tight">{{ $bimbingan->mahasiswa->name }}</h3>
                                <p class="text-xs text-gray-500 mt-0.5">Mahasiswa</p>
                            </div>
                        </div>
                        <span class="px-2.5 py-1 rounded-full text-xs font-semibold {{ $bimbingan->status == 'approved' ? 'bg-green-100 text-green-700' : ($bimbingan->status == 'rejected' ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700') }}">
                    ...
                    <div class="space-y-3 mb-6">
                        ...
                        <div class="flex items-center text-sm text-gray-600">
                            ...
                            {{ \Carbon\Carbon::parse($bimbingan->waktu_mulai)->format('H:i') }} - {{ \Carbon\Carbon::parse($bimbingan->waktu_selesai)->format('H:i') }} WIB
                        </div>

                        <div class="flex gap-2 mt-2">
                            @if($bimbingan->link_meet)
                                <a href="{{ $bimbingan->link_meet }}" target="_blank" class="text-xs flex items-center text-blue-600 bg-blue-50 px-2 py-1 rounded hover:bg-blue-100" title="Link Zoom tersedia">
                            ...
                            @if($bimbingan->file_prosedur)
                                <a href="{{ asset('storage/'.$bimbingan->file_prosedur) }}" target="_blank" class="text-xs flex items-center text-purple-600 bg-purple-50 px-2 py-1 rounded hover:bg-purple-100" title="Prosedur tersedia">
                ...
                <div class="bg-gray-50 px-6 py-4 border-t border-gray-100 grid grid-cols-2 gap-3">
                    ...
                    @else
                        <button disabled class="w-full flex items-center justify-center px-4 py-2 border border-gray-200 text-sm font-medium rounded-lg text-gray-400 bg-gray-100 cursor-not-allowed">
                            Link & Info
                        </button>
                    @endif
                </div>

                <div x-show="showStatusModal" style="display: none;"
                    class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
                    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                        <div x-show="showStatusModal" x-transition.opacity class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" aria-hidden="true"></div>
                        ...
                        <div x-show="showStatusModal" @click.away="showStatusModal = false" x-transition.scale
                            ...
                            <form action="{{ route('dosen.bimbingan.updateStatus', $bimbingan->id) }}" method="POST">
                                ...
                                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                                    <h3 class="text-lg leading-6 font-medium text-gray-900 mb-4" id="modal-title">Update Status Bimbingan</h3>
                                    <div class="space-y-4">
                                        <div>
                                            <label class="block text-sm font-medium text-gray-700 mb-1">Status</label>
                                            <select name="status" class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 sm:text-sm">
                                        ...
                                        <div>
                                            ...
                                            <textarea name="catatan_dosen" rows="3" class="shadow-sm focus:ring-green-500 focus:border-green-500 block w-full sm:text-sm border-gray-300 rounded-md" placeholder="Berikan catatan untuk mahasiswa...">{{ $bimbingan->catatan_dosen }}</textarea>
                                        </div>
                                    </div>
                                </div>
                                <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                                    ...
                                    <button type="button" @click="showStatusModal = false" class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">Batal</button>
                                </div>
                            </form>
                        </div>
                    </div>
                ...
                <div x-show="showDetailModal" style="display: none;"
                    class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
                    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                        <div x-show="showDetailModal" x-transition.opacity class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" aria-hidden="true"></div>
                        <span class="hidden sm:inline-block sm:align-middle sm:h-screen" aria-hidden="true">&#8203;</span>
                        <div x-show="showDetailModal" @click.away="showDetailModal = false" x-transition.scale
                            class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg w-full">

                            <form action="{{ route('dosen.bimbingan.updateDetails', $bimbingan->id) }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                                    <div class="flex items-center gap-3 mb-4 border-b pb-2">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-gray-500">Tambahkan link zoom dan prosedur untuk mahasiswa.</p>
                                        </div>
                                    </div>

                                    <div class="space-y-4">
                                        <div>
                                            <label class="block text-sm font-medium text-gray-700 mb-1">Link Zoom / Google Meet</label>
                                            <div class="mt-1 flex rounded-md shadow-sm">
                                                <span class="inline-flex items-center px-3 rounded-l-md border border-r-0 border-gray-300 bg-gray-50 text-gray-500 text-sm">
                                                    https://
                                                </span>
                                                <input type="text" name="link_meet" value="{{ $bimbingan->link_meet }}"
                                        ...
                                        <div>
                                            <label class="block text-sm font-medium text-gray-700 mb-1">Upload Prosedur (Gambar)</label>
                                            <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md hover:bg-gray-50 transition">
                                                <div class="space-y-1 text-center">
                                                    @if($bimbingan->file_prosedur)
                                                        <div class="mb-2">
                                                            <p class="text-xs text-green-600 font-semibold">File saat ini tersimpan:</p>
                                                            <img src="{{ asset('storage/'.$bimbingan->file_prosedur) }}" class="h-20 mx-auto object-cover rounded shadow-sm">
                                                        </div>
                                                    @else
                                                    ...
                                                    <div class="text-sm text-gray-600">
                                                        <label for="file-upload-{{$bimbingan->id}}" class="relative cursor-pointer bg-white rounded-md font-medium text-green-600 hover:text-green-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-green-500">
                                                            <span>Upload file baru</span>
                                                            <input id="file-upload-{{$bimbingan->id}}" name="file_prosedur" type="file" class="sr-only" accept="image/*">
                                                        </label>
                                                    </div>
                                ...
                                <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                                    <button type="submit" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-green-600 text-base font-medium text-white hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 sm:ml-3 sm:w-auto sm:text-sm">Simpan Detail</button>
                                    <button type="button" @click="showDetailModal = false" class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">Batal</button>
                                </div>
                            </form>
                        </div>
                    </div>
        ...
        @empty
            <div class="col-span-1 md:col-span-2 xl:col-span-3 text-center py-12 bg-white rounded-lg border border-dashed border-gray-300">
